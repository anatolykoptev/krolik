...
from krolik import __version__, __logo__

app = typer.Typer(
    name="krolik",
    help=f"{__logo__} krolik - Proactive AI Bot",
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} krolik v{__version__}")
        raise typer.Exit()
...
@app.command()
def onboard():
    """Initialize krolik configuration and workspace."""
    from krolik.config.loader import get_config_path, save_config
    from krolik.config.schema import Config
    from krolik.utils.helpers import get_workspace_path
...
I am Krolik, a proactive AI assistant.
...
def gateway(
    ...
    """Start the krolik gateway."""
    from krolik.config.loader import load_config, get_data_dir
    ...
    from krolik.cron.types import CronJob
    from krolik.heartbeat.service import HeartbeatService
    
    if verbose:
        import logging
        logging.basicConfig(level=logging.DEBUG)
    
    console.print(f"{__logo__} Starting krolik gateway on port {port}...")
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from krolik.bus.events import OutboundMessage
...
def agent(
    ...
    from krolik.config.loader import load_config
    from krolik.bus.queue import MessageBus
    from krolik.providers.litellm_provider import LiteLLMProvider
    from krolik.agent.loop import AgentLoop
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".krolik" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # krolik/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall krolik")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from krolik.config.loader import get_data_dir
    from krolik.cron.service import CronService
...
def env(
    ...
    from krolik.config.loader import find_env_file, load_env_file
    ...
    console.print("  3. ~/.nanobot/.env (legacy)")
...
def status():
    ...
    console.print(f"{__logo__} krolik Status\n")
